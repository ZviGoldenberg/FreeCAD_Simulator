...
def Gimbal(doc, dim):
    ...
    g001.Length, g001.Width, g001.Height = dim[1], dim[0], dim[2]
    ...
    g002 = body.newObject('PartDesign::AdditiveBox', 'G002')
    g002.Length, g002.Width, g002.Height = dim[1], dim[9], dim[6]
    g002.Support, g002.MapMode = xz, 'ObjectXY'
    g002.AttachmentOffset.Base = Vector(-dim[1] / 2, -dim[9], -dim[0] + dim[3])
    g003 = body.newObject('PartDesign::AdditiveCylinder', 'G003')
    g003.Radius, g003.Height = dim[1] / 2, dim[6]
    g003.Support, g003.MapMode = xz, 'ObjectXY'
    g003.AttachmentOffset.Base = Vector(0, 0, -dim[0] + dim[3])
    g003.Refine = True

    if dim[12] > 0:
        g004 = body.newObject('PartDesign::AdditiveCylinder', 'G004')
        g004.Radius, g004.Height = dim[11], dim[12]
        g004.Support, g004.MapMode = xz, 'ObjectXY'
        g004.AttachmentOffset.Base = Vector(0, 0, -dim[0] + dim[3] - dim[12])
        g005 = body.newObject('PartDesign::SubtractiveCylinder', 'G005')
        g005.Radius, g005.Height = 5, dim[12]
        g005.Support, g005.MapMode = xz, 'ObjectXY'
        g005.AttachmentOffset.Base = Vector(0, 0, -dim[0] + dim[3] - dim[12])
        g006 = Mark(doc)
        g006.Support, g006.MapMode = xz, 'FlatFace'
        g006.AttachmentOffset = Placement(Vector(0, -dim[11] + 5, -dim[0] + dim[3] - dim[12] - 0.001), Vector(0, 0, 1), -90)
        body.addObject(g006)
        g007 = doc.copyObject(g006, False)
        g007.AttachmentOffset.Base = Vector(0, -dim[11] + 5, -dim[0] + dim[3] + dim[6] + 0.001)
        body.addObject(g007)
    else:
        g004 = body.newObject('PartDesign::SubtractiveCylinder', 'G004')
        g004.Radius, g004.Height = dim[11], dim[0]
        g004.Support, g004.MapMode = xz, 'ObjectXY'
        g004.AttachmentOffset.Base = Vector(0, 0, -dim[0] + dim[3])
        g005 = RotaryScale(doc, 'G005', dim[11])
        g005.Support, g005.MapMode = xz, 'FlatFace'
        g005.AttachmentOffset = Placement(Vector(0, 0, -dim[0] + dim[3] - 0.001), Vector(0, 0, 1), 0)
        body.addObject(g005)
        g006 = doc.copyObject(g005, False)
        g006.AttachmentOffset.Base = Vector(0, 0, -dim[0] + dim[3]  + 2 * dim[6] + 0.001)
        body.addObject(g006)
        g007 = Mark(doc)
        ...
        g008.AttachmentOffset.Base = Vector(0, -dim[11] - 5, -dim[0] + dim[3] + 2 * dim[6] + 0.001)
        body.addObject(g008)
...
def YBase(doc, dim):
    ...
    box.AttachmentOffset.Base = Vector(-dim[1] + dim[3], -dim[4], -dim[5])
    body.ViewObject.Visibility = False
    return body
...
def Bed(doc, dim):
    ...
    box2.Length, box2.Width, box2.Height = dim[3], dim[4], dim[5] - dim[7]
    box2.Support, box2.MapMode = doc.getObject('XY_Plane'), 'ObjectXY'
    box2.AttachmentOffset.Base = Vector(dim[8] - dim[0] + dim[10], -dim[1] / 2, -dim[9])
    box3 = body.newObject('PartDesign::AdditiveBox', 'BBox3')
    box3.Length, box3.Width, box3.Height = dim[3], dim[4], dim[5] - dim[7]
    box3.Support, box3.MapMode = doc.getObject('XY_Plane'), 'ObjectXY'
    box3.AttachmentOffset.Base = Vector(dim[8] - dim[0] + dim[10], dim[1] / 2 - dim[4], -dim[9])
    box4 = body.newObject('PartDesign::AdditiveBox', 'BBox4')
    box4.Length, box4.Width, box4.Height = dim[3], dim[1], dim[7]
    box4.Support, box4.MapMode = doc.getObject('XY_Plane'), 'ObjectXY'
    box4.AttachmentOffset.Base = Vector(dim[8] - dim[0] + dim[10], -dim[1] / 2, -dim[9] + dim[5] - dim[7])
    box4.Refine = True
    body.ViewObject.Visibility = False
    return body
...
def Build(doc, g, b):
    ...
    workpiece = Workpiece(doc, (g, 70.91708, 36, 20, 3, 40, 7.5, 22))
    # see file:///C:\Projects\ACS\5axes\AcsStageModel\FreeCad\acsstage.pdf
    indexer = Indexer(doc, (60, 65, g))
    gimbal = Gimbal(doc, (178, 130, 25, 79, -25, 60, 20, 40, 50, g + 65, 49, 32.5, 20, 0))
    gimbalframe = Gimbal(doc, (
    198, 130, 25, 79, 0, 0, 20, 65, 0, g + 65 + 25, 49, 32.5, -20, math.sqrt(77 * 77 + 130 * 130 / 4.)))
    gimbalframe.Label = 'GimbalFrame'
    ybase = YBase(doc, (198, 750, 40, 275, 79, g + 65 + 25 + 40))
    head = Head(doc, (50, 55, 39, 80, 39, 202, 158, 154))
    # bed = Bed(doc, (750,550,250,200,200,837,112,158,720,275,102))
    bed = Bed(doc, (750, 1000, 250, 130, 130, 700, 312, 300, 275, g + 65 + 25 + 40, 150))
...
class Machine:
    def __init__(self, doc, axesinfo):
        ...
        self.doc = doc
        self.beamlength = 52
        self.g = 12  # distance between the center of table and A rotation axis
    ...
    def A(self, value):
        # print('A:', value)
        if (value is not None) and (not np.isnan(value)) and (value != self.A):
    ...
    @property
    def XYZAC(self):
        return (self.X, self.Y, self.Z, self.A, self.C, self.BeamLength)

    @XYZAC.setter
    def XYZAC(self, value):
        while len(value) < 6: value.append(None)
        self.X, self.Y, self.Z, self.A, self.C, self.BeamLength = value
    ...
    def GetKinematicMatricesByPrincipal1(self, b, c):
        b, c = b * math.pi / 180, c * math.pi / 180
        sb, cb, sc, cc = math.sin(b), math.cos(b), math.sin(c), math.cos(c)
        direct = np.array(
            [[cb * cc, cb * sc, -sb, 0], 
             [-sc, cc, 0, 0], 
             [sb * cc, sb * sc, cb, self.BeamLength],
             [0, 0, 0, 1]], dtype=float)
        feedback = np.array(
            [[cb * cc, -sc, sb * cc, -self.BeamLength * sb * cc],
             [cb * sc, cc, sb * sc, -self.BeamLength * sb * sc],
             [-sb, 0, cb, -self.BeamLength * cb],
             [0, 0, 0, 1]], dtype=float)
        return direct, feedback

    def GetKinematicMatricesByMachine(self, a, c):
        a, c = a * math.pi / 180, c * math.pi / 180  # Convert to radians
        sa, ca, sc, cc = math.sin(a), math.cos(a), math.sin(c), math.cos(c)
        G, L = self.g, self.BeamLength
        direct = np.array(
            [[cc, sc, 0, 0], 
             [-ca * sc, ca * cc, sa, -G * sa], 
             [sa * sc, -sa * cc, ca, -G * ca + L],
             [0, 0, 0, 1]], dtype=float)
        feedback = np.array(
            [[cc, -ca * sc, sa * sc],
             [sc, ca * cc, -sa * cc], 
        ...
        v = direct[:3, [3]]
        print(v)
        feedback = np.append(feedback, -feedback @ direct[:3, [3]], 1)
...
class MachineGui(QtGui.QMainWindow):
    ...
    def initUI(self):
        ...
        self.setWindowTitle("Santech Stage v.10")
    ...
    def closeEvent(self, event):
        App.closeDocument('SantechStage')
    ...
    def ZeroAll(self):
        ...
        self.XYZAC = (0, 0, Z0, 0, 0, None)
        self.machine.XYZAC = (0, 0, Z0, 0, 0, None)
    ...
    def Spherical2projections1(self, spheric):
        theta, phi = spheric * math.pi / 180
        st, ct, sp, cp = math.sin(theta), math.cos(theta), math.sin(phi), math.cos(phi)
        return np.array((st * cp, st * sp, ct))

    def Projections2spherical1(self, proj, store, ssingular):
        # print('proj:', proj)
        d = np.linalg.norm(proj)
        dx, dy, dz = proj / d
        # print(dx, dy, dz, d, prephi)
        theta = math.acos(dz) * 180.0 / math.pi
        singular = dz > 0.9999  # ~0.5 degree
        if singular:
            if not ssingular:
                ssingular.append(False)
                for i in range(4): store.append([theta, 0])
            if not ssingular[0]:
                self.sinvel = store[0][1] - store[1][1]
                self.sinacc = store[0][1] - 2*store[1][1] + store[2][1]
            sinvel = self.sinvel + self.sinacc
            phi = store[0][1] + 0.5 * (sinvel + self.sinvel)
            self.sinvel = sinvel
            a0 = store[0][0] - 2*store[1][0] + store[2][0]
            a1 = -2*store[0][0] + store[1][0]
            if abs(theta + a1 - a0) > abs(-theta + a1 - a0):
                theta = -theta
            print('singular spher:', theta, phi)
        else:
            phi = math.atan2(dy, dx) * 180.0 / math.pi 
            if not ssingular:
                ssingular.append(False)
                for i in range(4): store.append([theta, phi])
            if abs(phi - store[0][1]) > 90:
                theta = -theta
                phi = phi + 180 if phi < 0 else phi - 180
            print('spher:', theta, phi)
        for i in range(3): store[3-i] = store[2-i]
        store[0] = [theta, phi]
        ssingular[0] = singular
        return theta, phi
    ...
    def GetFixtureMatrices(self, offs):
        ...
        direct = np.array(
            [[cb * cc, -cb * sc, sb, x], 
             [ca * sc + sa * sb * cc, ca * cc - sa * sb * sc, -sa * cb, y],
             [sa * sc - ca * sb * cc, sa * cc + ca * sb * sc, ca * cb, z], 
             [0, 0, 0, 1]], dtype=float)
        print(direct)
        feedback = np.array(
            [[cb * cc, ca * sc + sa * sb * cc, sa * sc - ca * sb * cc],
             [-cb * sc, ca * cc - sa * sb * sc, sa * cc + ca * sb * sc], 
             [sb, -sa * cb, ca * cb]], dtype=float)
        feedback = np.append(feedback, -feedback @ [[x], [y], [z]], 1)
        feedback = np.append(feedback, [[0.0, 0.0, 0.0, 1.0]], 0)
        return direct, feedback
    ...
    def Execute(self):
        ...
        self.Run = not self.Run
        if not self.Run: return
        vstore, vsingular, fstore, fsingular = [], [], [], []
        with open(self.directory + '\\' + self.pbox.text(), 'r', newline='') as f:
            ...
            if self.usedll.isChecked():
                ...
                part_dll = np.array(vpart[:5], np.float64)
                partout_dll = np.array(vpart[:5], np.float64)
                machine_dll = np.array(self.machine.XYZAC[:5], np.float64)
                res = self.lib.FirstFeedbackTransform(5, machine_dll, 5, part_dll, partout_dll)
            while row:
                if not self.Run: return
                ...
                if coor == 'part':
                    ...
                    if self.usedll.isChecked():
                        ...
                        res = self.lib.DoDirectTransform(5, part_dll, 5, machine_dll)
                        feedback_dll = np.array([0, 0, 0, 0, 0], np.float64)
                        res = self.lib.DoFeedbackTransform(5, machine_dll, 5, feedback_dll)
                        vmachine = machine_dll
                        fpart = feedback_dll
                    else:
                        proj = self.Spherical2projections1(vpart[3:5])
                        t = np.vstack((np.vstack((vpart[:3], proj)).T, (1.0, 0.0)))
                        print(t)
                        t = fixture @ t
                        print(t)
                        theta, phi = self.Projections2spherical1(t[:3, 1], vstore, vsingular)
                        print('theta, phi:', (theta, phi))
                        vprincipal = np.append(t[:3, 0], [theta, phi])
                        print('principal:', vprincipal)
                        principal3, principal2, principal1 = principal2, principal1, vprincipal
                        phi = phi + math.pi / 2
                        kinematic, fkinematic = self.machine.GetKinematicMatricesByMachine(theta, phi)
                        # print('kinematic:', kinematic)
                        m = kinematic @ np.append(vprincipal[:3], 1.0)
                        vmachine = np.append(m[:3], [theta, -phi])
                        p = fkinematic @ m
                        fprincipal = np.append(p[:3], [vmachine[3], -vmachine[4]]) 
                        print('fprincipal:', fprincipal)
                        proj = self.Spherical2projections1(np.array(fprincipal[3:5]))
                        t = np.vstack((np.vstack((fprincipal[:3], proj)).T, (1.0, 0.0)))
                        t = ffixture @ t
                        # t = np.append(np.append(np.transpose(np.atleast_2d(fprincipal[:3])), proj, 1), [[1.0, 0.0, 0.0]], 0)
                        # t = ffixture @ t
                        theta, phi = self.Projections2spherical1(t[:3, 1], fstore, fsingular)
                        fpart = np.append(t[:3, 0], [theta, phi])
                elif coor == 'machine':
                    vmachine = vpart
                    ...
                    vmachine = vpart
                    fpart = vpart
                print('machine:', vmachine)
                print('fpart:', fpart)
                dvec = vpart - fpart
                if any(abs(d) > 1.e-6 for d in dvec): print('@@@@@@@@@@@@@@@@@@@@@@@@@@')
                vec = np.append(vmachine[:5], flen)
                # print('flen:', flen)
                self.XYZAC = vec
                self.machine.XYZAC = vec
                FreeCAD.Gui.updateGui()
...
def InitDll():
    ...
    if platform.architecture()[0] == "64bit":
        #lib = ctypes.CDLL(r'C:\Projects\ACS\5axes\AcsStageModel\FreeCad\TransformDll1.dll')
        lib = ctypes.CDLL(r'C:\Projects\ACS\5axes\TransformDll1\x64\Debug\TransformDll1.dll')
    ...
    lib.DoFeedbackTransform.argtypes = [ctypes.c_int, np.ctypeslib.ndpointer(dtype=np.float64), ctypes.c_int,
                                        np.ctypeslib.ndpointer(dtype=np.float64)]
    lib.FirstFeedbackTransform.restype = ctypes.c_int
    lib.FirstFeedbackTransform.argtypes = [ctypes.c_int, np.ctypeslib.ndpointer(dtype=np.float64), 
                                        ctypes.c_int, np.ctypeslib.ndpointer(dtype=np.float64), np.ctypeslib.ndpointer(dtype=np.float64)]
...
doc = App.newDocument('SantechStage')
axesinfo = (('X', -200, 200), ('Y', -200, 200), ('Z', 0, 350), ('A', -90, 90), ('C', -180, 180))  # motion limits
